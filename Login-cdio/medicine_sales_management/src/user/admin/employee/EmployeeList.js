...
export function EmployeeList() {
  ...
  useEffect(() => {
    if (!searchName) findAll();
    else findAllByName(searchName);
  }, [searchName]);
  const findAllByName = async (searchName) => {
    try {
      let temp = await axios.get(
        "http://localhost:8080/api/employee/listSearchName?name=" + searchName
      );
      console.log(temp);
      setEmployee((tempE) => (tempE = temp.data));
    } catch (e) {
      console.log(e);
    }
  };
      ...
      <div>
        ...
        <input
          ...
          onChange={(evt) => {
            ...
            setSearchName((name) => (name = evt.target.value));
          }}
        />
      </div>
      <div>
            ...
            <tbody>
              {employee?.map((item, index) => (
                <tr key={item.id_employee}>
                  ...
                  <td>{item.role[0].id === 1 ? "nhân viên" : "quản lý"}</td>
                  <td>
                    <NavLink
                      to={`/employee/update/${item.id_employee}`}
                      className="btn btn-primary"
                    >
                      update
                    </NavLink>
