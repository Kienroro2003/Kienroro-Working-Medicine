  ...
  return (
    <>
      ...
      <Formik
        initialValues={{
          ...bill,
          id_employee: JSON.stringify(employee[0].id_employee),
          id_member: JSON.stringify(member[0].id_member),
        }}
        onSubmit={(values, { setSubmitting }) => {
          ...
          const obj = {
            ...values,
            totalPayment: selectedMedicinePrice * values.quantity,
            id_employee: JSON.parse(values.id_employee),
            id_member: JSON.parse(values.id_member),
                  ...
                  <Field as="select" className="form-select" name="id_employee">
                    {employee?.map((item) => (
                      <option
                        ...
                        value={JSON.stringify(item.id_employee)}
                      >
                        {item.name}
                      </option>
                    ))}
                ...
                >
                  ...
                  <Field as="select" className="form-select" name="id_member">
                    {member?.map((item) => (
                      <option
                        key={item.id_member}
                        value={JSON.stringify(item.id_member)}
                      >
                        {item.nameMedicine}
                      </option>
                    ))}
                ...
                <div style={{ display: "inline-block", marginLeft: "20px" }}>
                  ...
                  <input
                    ...
                    value={
                      bill2
                        ? bill2.reduce(
                            (total, item) => total + item.totalPayment,
                            0
                          )
                        : 0
                    }
                      ...
                      <td>
                        <div className="mb-3">
                          <Field
                            ...
                            name="quantity"
                            onKeyUp={(evt) =>
                              setQuantityValue(evt.target.value)
                            }
                          />
                          <ErrorMessage
                            name="quantity"
                ...
                <tr key={item.id_bill}>
                  ...
                  <td>
                    <button
                      ...
                      onClick={() => handleIdNameDelete(item)}
